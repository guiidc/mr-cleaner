"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.seedRoles = void 0;
const connection_1 = __importDefault(require("../../src/database/connection"));
const rolesList = [
    { role: 'admin' },
    { role: 'user' }
];
function seedRoles() {
    return __awaiter(this, void 0, void 0, function* () {
        const promises = rolesList.map((role) => __awaiter(this, void 0, void 0, function* () {
            return (yield connection_1.default.role.upsert({
                where: { role: role.role },
                update: {},
                create: {
                    role: role.role
                }
            }));
        }));
        const insertedRoles = yield Promise.all(promises);
        insertedRoles.forEach((role) => {
            console.log(`🔑 Role ${role.role} inserted with id ${role.id}`);
        });
        return insertedRoles;
    });
}
exports.seedRoles = seedRoles;
